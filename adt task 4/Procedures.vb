...
Module Procedures
    ...
    Public Sub PrintCustomers(ByVal obj As Object) 'print customers based on obj provided

        If obj.GetType() = GetType(List(Of Customer)) Then
            Dim temp As IEnumerable(Of Customer) = obj
            temp = temp.OrderBy(Function(x) x.custID)
            For Each cust As Customer In temp   'should be it's own string function
        ...
        ElseIf obj.GetType() = GetType(Customer) Then
            ...
            Console.WriteLine("Total monthly bill (VAT @ 20%): {0}", temp.GetBill)
            Console.WriteLine("=========================================")
        End If
    ...
    Function LoadCustomers() As List(Of Customer)
        ...
        Using sr As System.IO.StreamReader = New System.IO.StreamReader(pathSave)
            lines = sr.ReadToEnd.Split(vbCrLf)
    ...
    Public Sub PrintBill()
        ...
        Else
            Do
                DisplayMenu("pb")
                Dim ch As Integer
                Dim searchedCust As New Customer
                        ...
                        Case 1
                            ...
                            For Each cust As Customer In CustomerList
                                If Int32.Parse(query) = cust.custID Then
                                    searchedCust = CustomerList.Find(Function(x) query = cust.custID)
                                    PrintCustomers(searchedCust)
                                    FoundCustomer = True
                                End If
                            Next
                        Case 2
                            ...
                            For Each cust As Customer In CustomerList
                                If query = cust.custName Then
                                    searchedCust = CustomerList.Find(Function(x) query = cust.custName)
                                    PrintCustomers(searchedCust)
                                    FoundCustomer = True
                                End If
                            Next
